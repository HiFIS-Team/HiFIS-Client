...
// 지점 관리 — 사이드바 글로벌 셀렉터에서 선택한 한 지점의 정보만 노출.
// 전체 지점 목록은 BranchProvider 의 ["branches"] 캐시에서 가져오고 (재요청 X),
// 그 안에서 selectedBranchId 로 한 건만 추려 렌더. SUPER_ADMIN 전용 (FC 는 가드).
export function BranchesContent() {
  ...
  // 글로벌 지점 — 사이드바 셀렉터. 선택된 지점 1 건만 보여줌.
  ...
  // 선택된 지점 1 건의 summary — 카운트 (회원/PT/예약)
  const summaryQuery = useQuery({
    queryKey: ["admin", "dashboard-summary", branch?.id ?? "none"],
    queryFn: () => getDashboardSummary(branch?.id ?? undefined),
    enabled: isSuper && !!branch,
    staleTime: 0,
  });
  const c = {
    members: summaryQuery.data?.members.total ?? 0,
    pts: summaryQuery.data?.pt_applications.total ?? 0,
    reservations: summaryQuery.data?.reservations.total ?? 0,
  };
  ...
  return (
    <div>
          ...
          <article className="rounded-xl border border-gray-200 p-5">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <RowActionButton onClick={() => setFormTarget(branch)}>
                  수정
                </RowActionButton>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onChange={(next) => {
                    ...
                    branchToggleMutation.mutate({ id: branch.id, next });
                  }}
                />
              </div>
            </div>
      ...
      {qrTarget && (
        <QrDialog branch={qrTarget} onClose={() => setQrTarget(null)} />
      )}
